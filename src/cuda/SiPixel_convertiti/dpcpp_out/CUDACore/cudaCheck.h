...
inline bool cudaCheck_(const char *file, int line, const char *cmd, int result,
                       const char *description = nullptr) {
  if (result == 0)
        return true;
...
inline bool cudaCheck_(const char *file, int line, const char *cmd, int result,
  ...
  */
  const char *error =
      "cudaGetErrorName not supported" /*cudaGetErrorName(result)*/;
  /*
  ...
  */
  const char *message =
      "cudaGetErrorString not supported" /*cudaGetErrorString(result)*/;
      abortOnCudaError(file, line, cmd, error, message, description);
      return false;
    }
