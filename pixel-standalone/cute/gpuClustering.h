...
void findClus(
 ...
 /* if (!((hist.size() / blockDim.x) <= maxiter)) {
  ...
  n40 = n60 = 0;
  __syncthreads();
  for (auto j = threadIdx.x; j < HistoContainer<uint16_t, nbins, maxPixInModule, 9, uint16_t><uint16_t, nbins, maxPixInModule, 9, uint16_t>::nbins(); j += blockDim.x) {
    if (hist.size(j) > 60)
      atomicAdd(&n60, 1);
    if (hist.size(j) > 40)
      atomicAdd(&n40, 1);
  ...
  for (unsigned int j = item_ct1.get_local_id(2), k = 0U; j < hist->size();
    ...
    int be = HistoContainer<uint16_t, nbins, maxPixInModule, 9, uint16_t>::bin(y[i] + 1);
    auto e = hist->end(be);
    ++p;
    if (!(0 == nnn[k])) {
      stream_ct1 << "error file gpuClustering";
