...
void __dpct_inline__ SYCL_EXTERNAL warpPrefixScan(T const *__restrict__ ci,
  ...
  auto x = ci[i];
  auto laneId = item_ct1.get_local_id(2) % 16;
#pragma unroll
...
__dpct_inline__ SYCL_EXTERNAL int blockPrefixScan(VT const *ci, VT *co,
  ...
  if (!(0 == item_ct1.get_local_range().get(2) %
                 16)) { // aggiungere il messaggio di errore!
    return -1;
  }
...
int multiBlockPrefixScan(T *const ici, T *ico, int32_t size, int32_t *pc,
  ...
  { // aggiungere il messaggio di errore! abort();
  }
  ...
  if (!(int(item_ct1.get_group_range().get(2)) ==
        *pc)) { // aggiungere il messaggio di errore!
    return -1;
  }
